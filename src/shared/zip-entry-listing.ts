...
export async function listZipEntriesWithZipInfo(archivePath: string): Promise<ArchiveEntry[]> {
  const proc = spawn(["zipinfo", "-v", archivePath], {
    stdout: "pipe",
    stderr: "pipe",
  })
  ...
  for (const chunk of chunks) {
    const nameMatch = chunk.match(/\n\s{2}(.+)\n\n\s+offset of local header/m)
    if (!nameMatch) continue

    const entryPath = nameMatch[1].trim()
    const attrMatch = chunk.match(/Unix file attributes \(([^)]+) octal\):/)
    const octalMode = attrMatch?.[1]

    const isSymlink = octalMode?.startsWith("120") ?? false
    const isDirectory = octalMode?.startsWith("040") ?? entryPath.endsWith("/")

    if (isSymlink) {
      const targetProc = spawn(["python3", "-c", [
        "import sys, zipfile",
      ...
      const [targetExitCode, targetStdout, targetStderr] = await Promise.all([
        targetProc.exited,
        new Response(targetProc.stdout).text(),
        new Response(targetProc.stderr).text(),
      ])

      if (targetExitCode !== 0) {
        throw new Error(`zip symlink target read failed (exit ${targetExitCode}): ${targetStderr}`)
      }

      entries.push({
        path: entryPath,
        type: "symlink",
        linkPath: targetStdout,
      })
      continue
    }
