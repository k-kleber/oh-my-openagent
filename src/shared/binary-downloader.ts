...
function parseTarEntry(line: string): ArchiveEntry | null {
  const match = line.match(/^([^\s])\S*\s+\S+\s+\d+\s+\d{4}-\d{2}-\d{2}\s+\d{2}:\d{2}\s+(.*)$/)
    ?? line.match(/^([^\s])\S*\s+\d+\s+\S+\s+\S+\s+\d+\s+\w+\s+\d+\s+(?:\d{2}:\d{2}|\d{4})\s+(.*)$/)
  if (!match) {
    return null
  }
...
async function listTarEntries(archivePath: string, cwd?: string): Promise<ArchiveEntry[]> {
  ...
  if (entries.length === 0 && stderr.trim().length > 0) {
    throw new Error(`tar entry listing failed: ${stderr}`)
  }
