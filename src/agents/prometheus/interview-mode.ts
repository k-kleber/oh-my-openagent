...
task(subagent_type="librarian", load_skills=["context7-mcp", "websearch-mcp"], prompt="I'm implementing [technology] in production and need authoritative guidance to avoid common mistakes. I'll use this for setup and configuration decisions. Use Context7 first for current official docs, then websearch for corroboration and OSS examples. Find official docs: setup, project structure, API reference, pitfalls, and migration gotchas. Also find 1-2 production-quality OSS examples (not tutorials). Skip beginner guides — I need production patterns only.", run_in_background=true)
\`\`\`
...
task(subagent_type="librarian", load_skills=["context7-mcp", "websearch-mcp"], prompt="I'm designing architecture for [domain] and need to evaluate trade-offs before committing. I'll use this to present concrete options to the user. Use Context7 first for library/framework docs when applicable, then websearch for architecture writeups and case studies. Find architectural best practices for [domain]: proven patterns, scalability trade-offs, common failure modes, and real-world case studies. Look at engineering blogs (Netflix/Uber/Stripe-level) and architecture guides. Skip generic pattern catalogs — I need domain-specific guidance.", run_in_background=true)
\`\`\`
...
task(subagent_type="librarian", load_skills=["context7-mcp", "websearch-mcp"], prompt="I'm implementing [Y] and need authoritative guidance to make correct API choices first try. I'll use this to follow intended patterns, not anti-patterns. Use Context7 first for current official docs, then websearch for gotchas and issue context. Find official docs: API reference, config options with defaults, migration guides, and recommended patterns. Check for 'common mistakes' sections and GitHub issues for gotchas. Return: key API signatures, recommended config, pitfalls.", run_in_background=true)
task(subagent_type="librarian", load_skills=["context7-mcp", "websearch-mcp"], prompt="I'm looking for battle-tested implementations of [Z] to identify the consensus approach. I'll use this to avoid reinventing the wheel. Use Context7 first for official API/docs context when the solution depends on external libraries, then websearch and OSS code search for production examples. Find OSS projects (1000+ stars) solving this — focus on: architecture decisions, edge case handling, test strategy, documented gotchas. Compare 2-3 implementations for common vs project-specific patterns. Skip tutorials — production code only.", run_in_background=true)
\`\`\`
...
\`\`\`typescript
task(subagent_type="librarian", load_skills=["context7-mcp", "websearch-mcp"], prompt="I'm integrating [library] and need to understand [specific feature] for correct first-try implementation. I'll use this to follow recommended patterns. Use Context7 first for current official docs, then websearch for changelog and ecosystem context. Find official docs: API surface, config options with defaults, TypeScript types, recommended usage, and breaking changes in recent versions. Check changelog if our version differs from latest. Return: API signatures, config snippets, pitfalls.", run_in_background=true)
\`\`\`
